...
import com.empireyard.spacemasters.handlers.CoOpMultiPlayerDataHandler;
...
public class CoOpMultiPlayer implements Screen, InputProcessor {
    //Debug
    ...
    CoOpMultiPlayerDataHandler multiPlayerDataHandler;

    byte[] mSendMessageBuffer = new byte[100];
    String INTERPOLATION_NAME = "fade";

    Vector2 player2ActualPosition;
    Vector2 player2TargetPosition;
    Vector2 position;
    float graphSize, steps, time = 0, duration = 2f;

    public CoOpMultiPlayer(SpaceMasters spaceMasters) {
        ...
        playerShips = new Array<PlayerShip>();
        playerShips.add(new PlayerShip(spaceMasters.getAssetHandler(), world, "player_ships", "ship", 1, "blue", 10, 10000));
        playerShips.get(0).getShipBody().setTransform(playerShips.get(0).getWidth(), playerShips.get(0).getHeight(), (float)Math.toRadians(0));

        playerShips.add(new PlayerShip(spaceMasters.getAssetHandler(), world, "player_ships", "ship", 1, "blue", 10, 10000));
        playerShips.get(1).getShipBody().setTransform(playerShips.get(1).getWidth(), playerShips.get(0).getHeight(), (float)Math.toRadians(0));
        ...
        SpaceMasters.gameStateManager.set(GameStateManager.GameState.PAUSE);
        ...
        multiPlayerDataHandler = new CoOpMultiPlayerDataHandler(playerShips, levelStateManager.peek().getEnemy().getShips());



        multiPlayerDataHandler.setLeadPlayerId(null);
        while(multiPlayerDataHandler.getLeadPlayerId() == null){
            SpaceMasters.getPlayServices().broadcastData(multiPlayerDataHandler.prepareBroadcastMessage(playerShips.get(0), levelStateManager.peek().getEnemy().getShips()), false);
            multiPlayerDataHandler.setLeadPlayerId(SpaceMasters.getPlayServices().getMyId(), SpaceMasters.getPlayServices().getPeerId());
        }

        playerShips.get(0).setName(SpaceMasters.getPlayServices().getMyName());
        playerShips.get(1).setName(SpaceMasters.getPlayServices().getPeerName());

        multiPlayerDataHandler.setHost();

        world.setContactListener(new WorldContactListener(world, playerShips, levelStateManager.peek().getEnemy().getShips(), levelStateManager.peek(), multiPlayerDataHandler.isHost()));


        if(multiPlayerDataHandler.isHost() == 1){
            playerShips.get(0).getShipBody().setTransform(
                    playerShips.get(0).getWidth(),
                    playerShips.get(0).getHeight(),
                    0
            );

            playerShips.get(1).getShipBody().setTransform(
                    mapPixelWidth - playerShips.get(1).getWidth(),
                    playerShips.get(1).getHeight(),
                    0
            );

        }else if(multiPlayerDataHandler.isHost() == 0) {
            playerShips.get(0).getShipBody().setTransform(
                    mapPixelWidth - playerShips.get(0).getWidth(),
                    playerShips.get(0).getHeight(),
                    0
            );

            playerShips.get(1).getShipBody().setTransform(
                    playerShips.get(1).getWidth(),
                    playerShips.get(1).getHeight(),
                    0
            );
        ...
        for(EnemyShip enemyShip : levelStateManager.peek().getEnemy().getShips()){
            enemyShip.setHost(multiPlayerDataHandler.isHost());
        }
    ...
    public void update(float dt){
        ...
        multiPlayerDataHandler.implementReceivedData(SpaceMasters.getPlayServices().getReceivedMessageBuffer(), playerShips.get(1), levelStateManager.peek().getEnemy().getShips());
...
//            );
    ...
    public void render(float delta) {
        if(SpaceMasters.gameStateManager.peek() == GameStateManager.GameState.PLAY) {
            update(delta);
